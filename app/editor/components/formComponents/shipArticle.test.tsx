...
describe("ShipArticleForm", () => {
  ...
  it("displays a notification when state is updated", async () => {
    (useActionState as Mock).mockReturnValue([{ text: "Success message" }, vi.fn(), false]);
    render(<ShipArticleForm />);
  ...
  it("closes the modal when the background is clicked", async () => {
    ...
    fireEvent.click(screen.getByTestId("delete-article-modal").querySelector(".modal-background")!);
    await waitFor(() => {
